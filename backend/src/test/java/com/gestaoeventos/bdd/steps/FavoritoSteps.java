...
public class FavoritoSteps {
    ...
    public void usuario_navegando_logado() {
        ...
        when(pessoaRepository.findById(this.usuario.getCpf())).thenReturn(Optional.of(this.usuario));
    }
    ...
    public void clica_icone_favoritar_evento() {
        ...
        when(eventoRepository.findById(this.evento.getId())).thenReturn(Optional.of(this.evento));

        when(favoritoRepository.findByPessoaCpfAndEventoId(this.usuario.getCpf(), this.evento.getId()))
                .thenReturn(Optional.empty());

        mensagemRetorno = favoritoService.toggleFavorito(this.usuario.getCpf(), this.evento.getId());
    }
    ...
    public void evento_salvo_na_area(String area) {
        ...
        assertEquals("Meus Favoritos", area, "A área mencionada na feature deveria ser 'Meus Favoritos'");
    }
    ...
    public void evento_removido_da_lista() {
        verify(favoritoRepository, times(1)).delete(this.favoritoExistente);
    }
    ...
    public void usuario_possui_eventos_favoritados() {
        ...
        when(favoritoRepository.findAllByPessoaCpf(this.usuario.getCpf()))
    ...
    public void usuario_solicita_lista() {
        listaFavoritosRetornada = favoritoService.listarFavoritos(this.usuario.getCpf());
    }

    @Entao("o sistema deve retornar a lista contendo os eventos")
    public void sistema_retorna_lista_eventos() {
        assertNotNull(listaFavoritosRetornada, "A lista de favoritos não deveria ser nula.");
        assertFalse(listaFavoritosRetornada.isEmpty(), "A lista deveria conter eventos.");
        assertEquals(1, listaFavoritosRetornada.size(), "A lista deveria ter 1 evento.");
    ...
    public void retornar_mensagem(String mensagemEsperada) {
        assertEquals(mensagemEsperada, mensagemRetorno, "A mensagem recebida pelo Service foi diferente da esperada.");
    }
}