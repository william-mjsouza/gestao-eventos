...
public class EventoSteps {
    ...
    public void usuario_e_organizador_valido() {
        ...
        organizador.setOrganizador(true); // Perfil correto

        // Resetando a exceção para garantir que um cenário não interfira no outro
        excecao = null;

        when(pessoaRepository.findById(organizador.getCpf())).thenReturn(Optional.of(organizador));
    ...
    public void preenche_campos_com_data_futura() {
        ...
        evento.setDataHoraInicio(LocalDateTime.now().plusDays(30)); // Data no futuro
    ...
    public void tenta_criar_evento_com_data_passada() {
        ...
        // Configurando data no passado:
        evento.setDataHoraInicio(LocalDateTime.now().minusDays(5));

        // Adicionando lote para garantir que o erro não seja o de falta de lote
