...
public class ListaEsperaSteps {
    ...
    public void ingresso_adicionado_ao_carrinho_do_primeiro() {
        ...
        primeiroDaFila.setSaldo(300.0);
        primeiroDaFila.setOrganizador(false);

        when(pessoaRepositorio.findById(primeiroDaFila.getCpf())).thenReturn(Optional.of(primeiroDaFila));
    ...
    public void finaliza_pagamento_dentro_do_prazo() {
        try {
            resultado = listaEsperaServico.confirmarPagamento(primeiroDaFila.getCpf(), evento.getId());
        } catch (Exception e) {
            excecao = e;
        }
    }
    ...
    public void inscricao_deve_ser_confirmada() {
        ...
        verify(listaEsperaRepositorio, atLeastOnce()).save(entradaPrimeiro);
    }
    ...
    public void ingresso_alocado_no_carrinho() {
        ...
        when(pessoaRepositorio.findById(segundoDaFila.getCpf())).thenReturn(Optional.of(segundoDaFila));
        ...
        when(listaEsperaRepositorio.findByEventoIdAndStatusOrderByPosicaoAsc(evento.getId(), StatusListaEspera.AGUARDANDO))
